...
So we can replace $\eqref{eq:freq_resp}$ as, 
   ...
   \end{align}
   Now we can expand the second term of above expression with an infinite geometric series,
   \begin{align}
   ...
   Now to get $h(n)$ for $n <5$ we will compare $\eqref{eq:longdiv_exp}$ with the below equation,
    ...
    \end{align}
    $h(n)$ will be the coefficient of $z^{-n}$ using this, from $\eqref{eq:longdiv_exp}$ we can write,
     \begin{align}
      h(0) &= 1\\
      h(1) &= \frac{-1}{2}\\
      h(2) &= \frac{5}{4}\\
...
wget https://github.com/Charanyash/EE3900-Digital_Signal_Processing/blob/main/Assignment-1/Codes/hn.py
     ...
     \begin{figure}[ht!]
      ...
      \label{hn}
     \end{figure}
    From the plot $\ref{hn}$, we can say that $h\brak{n}$ is bounded and converges to $0$ as $n$ increases.And theoritically we can the same using $\eqref{eq:hn}$,
     \begin{align}
       h\brak{n} &= \begin{cases}
                        0 &, n \leq 0\\
                        \brak{\frac{-1}{2}}^n &, 0<n<2\\
                        5\brak{\frac{1}{2}}^n &, n \geq 2
                     \end{cases}
     \end{align}
     As $5\brak{\frac{1}{2}}^n \rightarrow 0$ as $n \rightarrow \infty$ we can say $h\brak{n}$ is bounded and Convergent.
